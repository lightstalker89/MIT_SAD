    ...
    public class Program
    {
        ...
        private static void StartApplicationParts(string[] args)
        {
            ...
            if (CommandArg == 'p' || CommandArg == 'z')
            {
                ...
                {
                    Container.GetService<ILogger>().Log("Erronous xml file, please check file!", MessageType.Error);
                    Console.ReadKey();
                    Environment.Exit(0);
                }
            }
            ...
            {
                Container.GetService<ILogger>().Log("Unexpected parameters!", MessageType.Error);
                Container.GetService<IVisualizer>()
                    .WriteLine("Please start application with path to config xml file, or without any parameters!");
                Environment.Exit(0);
            }
        }
    }
