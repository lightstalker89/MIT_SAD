...
namespace ConsoleBoxDataManager
{
    ...
    public class DataManager : IDataManager
    {
        ...
        {
            return this.fileManager.ParallelSync;
        }

        /// <inheritdoc/>
        public void SetParallelSync(bool parallelSync)
        {
        ...
        /// <inheritdoc/>
        public void SetBlockCompareSizeInMb(long blockCompareSizeInMb)
        {
            this.fileManager.BlockCompareSizeInMb = blockCompareSizeInMb;
        }

        /// <inheritdoc/>
        public void SetBlockSizeInMb(long blockSizeInMb)
        {
            this.fileManager.BlockSizeInMb = blockSizeInMb;
        }
                ...
                if (this.QueueItems.Count > 0)
                {
                    ...
                    if (this.QueueItems.TryDequeue(out currentElement))
                    {
                        ...
                        catch (IOException inoutException)
                        {
                            ...
                            {
                                if (currentElement.RepeatCount < 21)
                                {
                                    this.QueueItems.Enqueue(currentElement);
                                }
                                else
                                {
                                    this.ExceptionMessage(this, new ExceptionDataManagerEventArgs(inoutException.GetType(), string.Concat("File ", currentElement.Name, " could not be synchronised")));
                                }
                            }
